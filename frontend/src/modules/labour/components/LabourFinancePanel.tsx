...
export function LabourFinancePanel({ projectId, canManagePayments }: { projectId: string; canManagePayments: boolean }) {
  ...
  return (
    <div className="space-y-5">
      <Panel>
        <PanelHeader
          eyebrow="LABOUR FINANCE"
          title="Advances"
          description="Cash advances given against future wages."
          action={canManagePayments ? <Button variant="primary" size="sm" onClick={() => setAdvanceOpen(true)}>Record advance</Button> : null}
        />
        {(advancesQuery.data ?? []).length === 0 ? (
          <EmptyState icon="budget" title="No advances recorded" description="Advances given to workers or contractors will appear here." />
        ) : (
          <TableShell>
            <table className="w-full min-w-[500px] text-left">
              <thead className="bg-[var(--color-surface-muted)]"><tr className="text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-muted)]"><th className="px-4 py-3">Source</th><th className="px-4 py-3">Date</th><th className="px-4 py-3 text-right">Amount</th></tr></thead>
              <tbody>
      ...
      <Panel>
        <PanelHeader
          eyebrow="LABOUR FINANCE"
          title="Payments"
          description="Wage payments made, net of any advance recovery."
          action={canManagePayments ? <Button variant="primary" size="sm" onClick={() => setPaymentOpen(true)}>Record payment</Button> : null}
        />
        {(paymentsQuery.data ?? []).length === 0 ? (
          <EmptyState icon="budget" title="No payments recorded" description="Wage payments made to workers or contractors will appear here." />
        ) : (
          <TableShell>
            <table className="w-full min-w-[600px] text-left">
              <thead className="bg-[var(--color-surface-muted)]"><tr className="text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-muted)]"><th className="px-4 py-3">Source</th><th className="px-4 py-3">Period</th><th className="px-4 py-3 text-right">Gross</th><th className="px-4 py-3 text-right">Net paid</th></tr></thead>
              <tbody>
...
function AdvanceDialog({ projectId, sources, onClose }: { projectId: string; sources: { id: string; name: string }[]; onClose: () => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    createAdvance.mutate({ source_id: sourceId, amount: Number(amount), advance_date: advanceDate }, {
      ...
      onError: (mutationError) => setError(getApiErrorMessage(mutationError, "Couldn't record this advance.")),
    });
  }
  ...
  return (
    <Modal title="Record advance" onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        <Field label="Source"><select className={inputClass} value={sourceId} onChange={(e) => setSourceId(e.target.value)}>{sources.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
        ...
        <div className="flex justify-end gap-2 border-t border-[var(--color-border)] pt-4">
          ...
          <Button type="submit" variant="primary" disabled={createAdvance.isPending || !sourceId || !amount}>{createAdvance.isPending ? "Saving…" : "Record advance"}</Button>
        </div>
      </form>
    </Modal>
  );
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    createPayment.mutate(
      { source_id: sourceId, period_start: periodStart, period_end: periodEnd, gross_wage_amount: Number(gross), advance_recovered_amount: Number(advanceRecovered), payment_date: paymentDate },
      {
        ...
        onError: (mutationError) => setError(getApiErrorMessage(mutationError, "Couldn't record this payment.")),
      },
    );
  }

  return (
    <Modal title="Record wage payment" description="Net paid = gross wages minus any advance recovered." onClose={onClose} wide>
      <form onSubmit={submit} className="space-y-4">
        <Field label="Source"><select className={inputClass} value={sourceId} onChange={(e) => setSourceId(e.target.value)}>{sources.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Gross wages (PKR)"><input type="number" min="0" step="any" required className={inputClass} value={gross} onChange={(e) => setGross(e.target.value)} /></Field>
        ...
        <div className="flex justify-end gap-2 border-t border-[var(--color-border)] pt-4">
          ...
          <Button type="submit" variant="primary" disabled={createPayment.isPending || !sourceId || !periodStart || !gross}>{createPayment.isPending ? "Saving…" : "Record payment"}</Button>
        </div>
      </form>
    </Modal>
  );
