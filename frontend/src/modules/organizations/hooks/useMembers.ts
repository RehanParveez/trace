...
export const memberKeys = {
  ...
  list: (skip: number, limit: number) =>
    [...memberKeys.all, "list", skip, limit] as const,
 
  detail: (userId: string) => [...memberKeys.all, "detail", userId] as const,
};
 
export function useMembers(skip = 0, limit = 100) {
  return useQuery({
...
export function useMember(userId?: string) {
  return useQuery({
    ...
    queryFn: () => organizationsApi.getMember(userId!),
...
export function useUpdateMemberRole() {
  ...
  return useMutation({
    ...
    }) => organizationsApi.updateMemberRole(userId, payload),
...
export function useUpdateMemberStatus() {
  ...
  return useMutation({
    ...
    }) => organizationsApi.updateMemberStatus(userId, payload),
