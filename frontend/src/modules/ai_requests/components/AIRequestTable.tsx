...
export function AIRequestTable({ entries }: AIRequestTableProps) {
  ...
  return (
    <>
      <Panel>
        <PanelHeader
          eyebrow={t("ai.table.eyebrow")}
          title={t("ai.table.title")}
        ...
        {entries.length === 0 ? (
          <EmptyState icon="settings" title={t("ai.table.emptyTitle")} description={t("ai.table.emptyDesc")} />
        ) : (
          <TableShell>
            <table className="w-full min-w-[700px] text-left">
              <thead className="bg-[var(--color-surface-muted)]">
                <tr className="text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-muted)]">
                  <th className="px-4 py-3">{t("ai.table.colWhen")}</th>
                  <th className="px-4 py-3">{t("ai.table.colPurpose")}</th>
                  <th className="px-4 py-3">{t("ai.table.colModel")}</th>
                  <th className="px-4 py-3">{t("ai.table.colStatus")}</th>
                  <th className="px-4 py-3 text-right">{t("ai.table.colLatency")}</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} onClick={() => setViewing(entry)} className="cursor-pointer border-t border-[var(--color-border)] transition hover:bg-[var(--color-surface-muted)]">
                    ...
                    <td className="px-4 py-3.5">
                      {entry.response ? <Badge tone={getAIStatusTone(entry.response.status)}>{entry.response.status}</Badge> : <Badge tone="slate">{t("ai.table.pending")}</Badge>}
                    </td>
      ...
      {viewing ? (
        <Modal title={t("ai.table.detailTitle")} description={formatAIPurpose(viewing.purpose)} onClose={() => setViewing(null)} wide>
          <div className="space-y-4">
            ...
            {viewing.response?.parsed_output ? (
              <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--color-text-muted)]">{t("ai.table.parsedOutput")}</div>
